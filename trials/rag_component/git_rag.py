...
class GitHubRAGPipeline:
    ...
    # Move the OpenAI import to the top of the file with other imports (around line 45)
    try:
        import openai
    ...
    def __init__(self,
        ...
        if self.use_openai:
            ...
            if api_key:
                ...
                logger.warning("OPENAI_API_KEY not found. Falling back to simple formatting.")
    ...
    def query_repository(self, query: str, top_k: int = 5, include_context: bool = True) -> str:
        ...
        # If OpenAI is available, use it for intelligent analysis
        if self.use_openai and self.openai_client:
            return self._query_with_openai(query, results, include_context)
        else:
            return self._query_with_simple_formatting(query, results, include_context)
    
    def _query_with_openai(self, query: str, results: List[Tuple], include_context: bool = True) -> str:
        """Use OpenAI to analyze retrieved chunks and provide intelligent responses."""
        try:
            ...
            for chunk, score in results:
                ...
                if chunk.metadata.get('name'):
                    chunk_info['name'] = chunk.metadata['name']
                if include_context and chunk.metadata.get('docstring'):
                    chunk_info['documentation'] = chunk.metadata['docstring']
                
                context_chunks.append(chunk_info)
            
            # Create prompt for OpenAI
            repo_info = f"Repository: {self.repo_info.get('full_name', 'Unknown')}"
            if self.repo_info.get('description'):
                repo_info += f"\nDescription: {self.repo_info['description']}"
            
            prompt = f"""You are an expert code analyst. A user is asking about a GitHub repository.

{repo_info}

User Query: {query}

I've retrieved the most relevant code chunks from the repository. Please analyze these chunks and provide a comprehensive, helpful answer to the user's query.

Relevant Code Chunks:
"""
            
            for i, chunk in enumerate(context_chunks, 1):
                prompt += f"\n\n--- Chunk {i} (Relevance: {chunk['relevance']}) ---\n"
                prompt += f"File: {chunk['file']}\n"
                prompt += f"Type: {chunk['type']} ({chunk['language']})\n"
                prompt += f"Lines: {chunk['lines']}\n"
                if 'name' in chunk:
                    prompt += f"Name: {chunk['name']}\n"
                prompt += f"\nCode:\n```{chunk['language']}\n{chunk['content']}\n```"
                if 'documentation' in chunk:
                    prompt += f"\n\nDocumentation: {chunk['documentation']}"
            
            prompt += """\n\nPlease provide:
1. A direct answer to the user's query
2. Relevant code explanations
3. Key insights about the repository structure or functionality
4. Any important patterns or architectural decisions you notice
...
Format your response in clear, well-structured markdown."""
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert software engineer and code analyst. Provide clear, accurate, and helpful analysis of code repositories."},
    ...
    def _query_with_simple_formatting(self, query: str, results: List[Tuple], include_context: bool = True) -> str:
        """Fallback method for simple formatting without LLM."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GitHub Repository RAG Pipeline with LLM')
